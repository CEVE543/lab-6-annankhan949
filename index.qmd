...
c = g["tas"];
dim_data = c[time=1]
# read into memory first using readcubedata, tutorial works in Julia REPL but not Quarto. readcubedata returns a 4D array not YAXA so Quarto throws error when trying to generate plots, swapped it to collecting only when plotting.

#Plot with custom color scale using scatter instead of heatmap for simplicity 
plt = data(dim_data |> collect) * mapping(:lon, :lat; color=:value)
...
plt = data(dim_time |> collect) * mapping(:lon, :lat; color = :value, layout = :time => nonnumeric)
...
```{julia}
plt = data((dim_data[lon=50..59]) |> collect) * mapping(:lat, :value => "tas"; color=:value => "tas",
...
```{julia} 
dim_series = c[time=DateTime("2015-01-01") .. DateTime("2015-01-04"), lon = 150 .. 157, lat = 0..1] #this could be useful for doing chains of transformations data |> filter |> sort |> collect
```

## Plotting and adding basic stat analysis
```{julia}
plt = data(dim_series |> collect) * mapping(:time, :value => "tas"; color=:lon => nonnumeric)
